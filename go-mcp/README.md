...
[MCP](https://modelcontextprotocol.io) server exposing the FreeDictionary SDK as
two agent tools — `free-dictionary_list` and `free-dictionary_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/free-dictionary-mcp)
make build

# 2. Provide credentials via the environment
export FREE_DICTIONARY_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user free-dictionary \
  -- /absolute/path/to/free-dictionary-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./free-dictionary-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// free-dictionary_list: first page of records
{ "entity": "entry" }
{ "entity": "entry", "query": { } }

// free-dictionary_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/free-dictionary-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user free-dictionary \
     -- "$PWD"/dist/*/free-dictionary-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `free-dictionary_list` and `free-dictionary_load` tools now appear
   in new sessions. Ask the agent to *"list entry using free-dictionary"*
   and it calls `free-dictionary_list` with `{"entity":"entry"}`.
...
```sh
export FREE_DICTIONARY_APIKEY=sk_live_xxx            # API key
export FREE_DICTIONARY_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `free-dictionary_list` tool
...
### Call the `free-dictionary_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `free-dictionary_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `free-dictionary_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `FREE_DICTIONARY_APIKEY` | API key sent with every request. |
| `FREE_DICTIONARY_BASE` | Optional override of the API base URL. |
...
```sh
./free-dictionary-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"free-dictionary_load","arguments":{"entity":"entry","query":{"id":1}}}}'
```
