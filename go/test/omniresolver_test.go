...
import (
	...
	omni "github.com/voxgig-sdk/free-dictionary-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.FreeDictionaryError); is {
				out := map[string]any{
					"name":    "FreeDictionaryError",
...
func omniCtx(arg any, client *sdk.FreeDictionarySDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.FreeDictionarySDK); is {
			client = live
