...
class EntryEntity
{
    ...
    {
        if ($args) {
            $this->_data = FreeDictionaryHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = FreeDictionaryHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   the SDK boundary; throws FreeDictionaryError on failure (item-5 convention).
     */
