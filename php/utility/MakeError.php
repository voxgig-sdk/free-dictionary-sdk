...
class FreeDictionaryMakeError
{
    public static function call(?FreeDictionaryContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new FreeDictionaryContext([], null);
        }
        $op = $ctx->op ?? new FreeDictionaryOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new FreeDictionaryResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof FreeDictionaryError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new FreeDictionaryError('', $msg, $ctx);
        ...
        if ($err instanceof FreeDictionaryError) {
