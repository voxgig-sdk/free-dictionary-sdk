...
// FreeDictionary SDK feature test
//
...
class FtCtrl extends FreeDictionaryControl
{
...
class FtClient
{
    ...
    private FreeDictionaryUtility $utility;

    public function __construct(FreeDictionaryUtility $utility, array $options)
    {
    ...
    public function get_utility(): FreeDictionaryUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FreeDictionaryContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public FreeDictionaryContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (FreeDictionaryContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FreeDictionaryConfig::make_feature($name);
    ...
    private static function buildUrl(FreeDictionarySpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FreeDictionaryError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FreeDictionaryResponse($fetched);
        }
    ...
    private function fail(FreeDictionaryContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FreeDictionaryContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
