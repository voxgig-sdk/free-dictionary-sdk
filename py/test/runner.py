# FreeDictionary SDK test runner
...
class FreeDictionaryTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            FreeDictionaryTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = FreeDictionaryTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = FreeDictionaryTestRunner.getenv("FREE_DICTIONARY_TEST_LIVE")
        override = FreeDictionaryTestRunner.getenv("FREE_DICTIONARY_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = FreeDictionaryTestRunner.getenv(key)
        ...
        explain = FreeDictionaryTestRunner.getenv("FREE_DICTIONARY_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["FREE_DICTIONARY_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if FreeDictionaryTestRunner._test_control is not None:
            return FreeDictionaryTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                FreeDictionaryTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            FreeDictionaryTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = FreeDictionaryTestRunner.load_test_control()
...
def load_env_local():
    FreeDictionaryTestRunner.load_env_local()


def env_override(m):
    return FreeDictionaryTestRunner.env_override(m)


def entity_data(v):
    return FreeDictionaryTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return FreeDictionaryTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return FreeDictionaryTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return FreeDictionaryTestRunner.load_test_control()


def live_client_options():
    return FreeDictionaryTestRunner.live_client_options()


def live_delay_ms():
    return FreeDictionaryTestRunner.live_delay_ms()
