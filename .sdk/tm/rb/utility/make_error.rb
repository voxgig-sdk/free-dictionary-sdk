...
module FreeDictionaryUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = FreeDictionaryContext.new({}, nil)
    end
    op = ctx.op || FreeDictionaryOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || FreeDictionaryResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(FreeDictionaryError) ? err.msg : err.to_s
    ...
    sdk_err = FreeDictionaryError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(FreeDictionaryError)
