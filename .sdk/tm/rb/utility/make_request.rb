...
module FreeDictionaryUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = FreeDictionaryResponse.new({})
    ...
    if fetch_err
      ...
      response = FreeDictionaryResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = FreeDictionaryResponse.new(fetched)
    else
