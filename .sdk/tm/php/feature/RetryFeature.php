...
// FreeDictionary SDK retry feature
...
class FreeDictionaryRetryFeature extends FreeDictionaryBaseFeature
{
    ...
    public function init(FreeDictionaryContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (FreeDictionaryContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(FreeDictionaryContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(FreeDictionaryContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
