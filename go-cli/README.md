...
boru-driven command-line client **and** interactive REPL for the FreeDictionary
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/free-dictionary-cli)
make build

# 2. See usage (words, entities, env vars)
./free-dictionary-cli --help

# 3. Provide credentials once, via the environment
export FREE_DICTIONARY_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./free-dictionary-cli list entry

# 5. Override the API base URL for a single call
FREE_DICTIONARY_BASE=https://api.example.com ./free-dictionary-cli list entry

# 6. No arguments -> interactive REPL
./free-dictionary-cli
free-dictionary> list entry
free-dictionary> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/free-dictionary-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/free-dictionary-cli list entry
   ```
...
Run with no arguments to open a REPL (prompt `free-dictionary>`). Each line is
evaluated as its own boru expression:

```text
$ ./free-dictionary-cli
free-dictionary> list entry
free-dictionary> /help
free-dictionary> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/free-dictionary-cli`. |
...
(`./free-dictionary-cli <expr>`) and interactively (the REPL), and expressions compose the
