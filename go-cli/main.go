// Command FreeDictionary-cli is a CLI + REPL for the FreeDictionary SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      FREE_DICTIONARY_APIKEY (api key), FREE_DICTIONARY_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: FREE_DICTIONARY_APIKEY carries the API key and
	// FREE_DICTIONARY_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("FREE_DICTIONARY_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("FREE_DICTIONARY_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewFreeDictionarySDK(opts)
