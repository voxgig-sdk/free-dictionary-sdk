# FreeDictionary SDK Entry entity
...
class EntryEntity
  ...
  def data_set(args)
    if args
      @_data = FreeDictionaryHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = FreeDictionaryHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #
  # @param reqmatch [EntryListMatch, Hash, nil] match filter (any subset of
  #   Entry fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Entry>, Array] the matching Entry items; raises FreeDictionaryError on failure
