...
require_relative 'FreeDictionary_types'
...
class FreeDictionarySDK
  ...
  def initialize(options = {})
    ...
    feature_opts = FreeDictionaryHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = FreeDictionaryHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, FreeDictionaryFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    FreeDictionaryUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = FreeDictionaryHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = FreeDictionaryHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = FreeDictionaryHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => FreeDictionaryError.new(
        "#{op}_allow",
        "FreeDictionarySDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue FreeDictionaryError => err
    ...
    if fetched.is_a?(Hash)
      status = FreeDictionaryHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = FreeDictionaryError.new(
        "graphql_error", "FreeDictionarySDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = FreeDictionarySDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
