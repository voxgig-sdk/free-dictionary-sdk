# FreeDictionary Ruby SDK



The Ruby SDK for the FreeDictionary API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Entry` — with named operations (`list`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
- Releases: [https://github.com/voxgig-sdk/free-dictionary-sdk/releases](https://github.com/voxgig-sdk/free-dictionary-sdk/releases)
...
```ruby
client = FreeDictionarySDK.test(testopts, sdkopts)
```
...
raise a `FreeDictionaryError` (a `StandardError` subclass), so wrap
...
```
rb/
├── FreeDictionary_sdk.rb       -- Main SDK module
...
The main module (`FreeDictionary_sdk`) exports the SDK class
