...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = FreeDictionaryOmni.make_runner(TEST_JSON_FILE, FreeDictionarySDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    FreeDictionaryError.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : FreeDictionaryOmni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = FreeDictionaryOperation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
