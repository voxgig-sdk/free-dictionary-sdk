...
class EntryDirectTest < Minitest::Test
  def test_direct_list_entry
    setup = entry_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-entry", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
    if setup[:live]
      ["language01", "word01"].each do |_live_key|
...
def entry_direct_setup(mockres)
  ...
  if live
    ...
    client = FreeDictionarySDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,
