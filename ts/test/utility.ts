...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.FREEDICTIONARY_TEST_LIVE ||
    'TRUE' === process.env.FREEDICTIONARY_TEST_OVERRIDE
  ) {
  ...
  m.FREEDICTIONARY_TEST_EXPLAIN = process.env.FREEDICTIONARY_TEST_EXPLAIN || m.FREEDICTIONARY_TEST_EXPLAIN
