...
//     (\`new FreeDictionarySDK(...)\` / \`FreeDictionarySDK.test(...)\` become
//     \`FreeDictionarySDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'FreeDictionarySDK'
...
// rewrite real client construction (\`new FreeDictionarySDK(...)\` and any
// \`FreeDictionarySDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+FreeDictionarySDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/FreeDictionarySDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
